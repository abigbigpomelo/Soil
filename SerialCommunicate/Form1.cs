...
namespace SerialCommunicate
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            this.zedGraphControl1.GraphPane.YAxis.Title.Text = "湿度值 %";
            ...
            myCurve = zedGraphControl1.GraphPane.AddCurve("湿度", list1, Color.Blue, SymbolType.None);
        ...
        private void port_DataReceived(object sender, SerialDataReceivedEventArgs e)//串口数据接收事件
        {
            ...
            if (readedDataBuffer.Count >= 8)
            {
                int removeCount = 0;
                for (int i = 0; i < readedDataBuffer.Count - 7; ++i)
                {
                        // 14 7B节点
                        if (readedDataBuffer[i] == 0xFD & readedDataBuffer[i + 4] == 0x11)
                        {
                            ...
                            int data31, data32;
                            float v3, T3,H;
                            data31 = readedDataBuffer[i + 5];//高八位
                            data32 = readedDataBuffer[i + 6];//低八位
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            string sql = "CREATE TABLE IF NOT EXISTS '" + "湿度" + uploadDate + "' (日期 varchar(6), 时间 varchar(6), 湿度 varchar(6));";//建表语句  
            SQLiteCommand cmdCreateTable = new SQLiteCommand(sql, conn);
            cmdCreateTable.ExecuteNonQuery();//如果表不存在，创建数据表  

            SQLiteCommand cmdInsert = new SQLiteCommand(conn);
            cmdInsert.CommandText = "INSERT INTO '" + "湿度" + uploadDate + "'  VALUES('" + uploadDate + "','" + uploadTime + "','" + wendu + "')";//插入几条数据  
